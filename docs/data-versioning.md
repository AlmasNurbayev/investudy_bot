...
**Запись** (`internal/repository`, потребляется `internal/parser`):
...
**Чтение** (`internal/repository`, потребляется `internal/report`; тот же интерфейс переиспользуется будущим веб-бэкендом):
