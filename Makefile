...
.PHONY: dev-bot
# TZ задаётся явно: бот считает границы месяца и квартала по зоне процесса,
# а на хосте она произвольная. В docker-compose.yml сервису задана та же зона,
# и локальный прогон должен отвечать то же самое, что и развёрнутый.
